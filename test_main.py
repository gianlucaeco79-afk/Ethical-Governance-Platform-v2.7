...
from ethical_governance.infra.alerts import AlertService, NotifierBase, WebhookNotifier
from ethical_governance.infra.audit import AuditLogger, AuditLoggerBase, FeedbackStore
...
class TestABCInterfaces:
    def test_audit_logger_base_is_abstract(self):
        from abc import ABC
        assert issubclass(AuditLoggerBase, ABC)

    def test_audit_logger_implements_base(self):
        assert issubclass(AuditLogger, AuditLoggerBase)

    def test_notifier_base_is_abstract(self):
        from abc import ABC
        assert issubclass(NotifierBase, ABC)
        assert "send" in NotifierBase.__abstractmethods__

    def test_webhook_notifier_implements_base(self):
        assert issubclass(WebhookNotifier, NotifierBase)

    def test_cannot_instantiate_notifier_base(self):
        with pytest.raises(TypeError): NotifierBase()

    def test_custom_notifier_accepted(self):
        class SlackNotifier(NotifierBase):
            async def send(self, payload) -> bool: return True
        assert isinstance(SlackNotifier(), NotifierBase)
...
class TestDriftDetector:
    ...
    def test_numeric_ks(self, det):
        ...
        assert r["x"]["test"] == "ks" and r["x"]["detected_type"] == "numeric"
...
class TestAlertService:
    ...
    async def test_no_alert_below_threshold(self, audit):
        ...
        fired = await svc.evaluate_and_fire("m","t",self._fairness(dpd=0.05),{},0.10)
        assert fired == []

    @pytest.mark.asyncio
    async def test_dpd_alert_fires(self, audit):
        svc = AlertService(WebhookNotifier(None), audit)
        fired = await svc.evaluate_and_fire("m","t",self._fairness(dpd=0.20),{},0.10)
        assert len(fired) >= 1
    ...
    async def test_webhook_failure_no_raise(self, audit):
        ...
        fired = await svc.evaluate_and_fire("m","t",self._fairness(dpd=0.25),{},0.10)
...
class TestRequestIdPropagation:
    @pytest.mark.asyncio
    async def test_request_id_in_auto_decision_log(self, audit):
    ...
    @pytest.mark.asyncio
    async def test_monitor_record_stores_request_id(self, monitor):
        rid = str(uuid.uuid4())
    ...
    @pytest.mark.asyncio
    async def test_monitor_record_default_none(self, monitor):
...
class TestAPIEndpoints:
    ...
    def test_model_not_found_structured_error(self, client, auth):
        ...
        assert r.json()["detail"]["error"] == "ModelNotFoundError"
    ...
    def test_feature_missing_structured_error(self, client, auth):
        ...
        assert r.json()["detail"]["error"] == "FeatureMissingError"
    ...
    def test_prediction_not_found_structured_error(self, client, auth):
        ...
        assert r.status_code == 404
        assert r.json()["detail"]["error"] == "PredictionNotFoundError"
    ...
    def test_compare_single_model_400(self, client, auth):
        r = client.get("/v1/compare?models=credit_model_v1", headers=auth)
        assert r.status_code == 400
